...
# verifica se a jogada é válida segundo as regras do Reversi
def validar_jogada(tabuleiro, l, c, jogador):
    ...
    # define o adversário
    adversario = branco if jogador == preto else preto
...
def contar_pecas(tabuleiro):
    pretas = np.sum(tabuleiro == preto)
    brancas = np.sum(tabuleiro == branco)
